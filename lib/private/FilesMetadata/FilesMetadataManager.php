...
use OCP\Files\Node;
use OCP\FilesMetadata\Event\MetadataEventBackgroundEvent;
use OCP\FilesMetadata\Event\MetadataEventLiveEvent;
...
class FilesMetadataManager implements IFilesMetadataManager {
	...
	public function refreshMetadata(
		...
		if ($asBackgroundJob) {
			$event = new MetadataEventBackgroundEvent($node, $metadata);
		} else {
			$event = new MetadataEventLiveEvent($node, $metadata);
		}
		...
		if ($event instanceof MetadataEventLiveEvent && $event->isRunAsBackgroundJobRequested()) {
...
//		$this->removeDeprecatedMetadata($filesMetadata);
	...
	public function getMetadataQuery(
		...
		$metadataQuery = new MetadataQuery($qb);
		$metadataQuery->leftJoinIndex($fileTableAlias, $fileIdField);
